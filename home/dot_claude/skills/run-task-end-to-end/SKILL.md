...
description: "Run one implementation task through guided no-argument intake, a natural-language prompt, or explicit checkout/worktree arguments, carrying it through policy-driven verification and optional publishing."
...
Run one task through the resolved workspace and policy:

```text
identity preflight -> validate -> read materials -> resolve workspace/policy -> prepare workspace
         -> task branch -> plan -> implement -> review -> verify/fix/retest
...
## Claude question presentation

The shared invocation reference owns question semantics, allowed values, defaults, ambiguity rules,
and provenance. When Claude Code's native structured question surface is available, use
`AskUserQuestion` for unresolved closed-set guided choices. Keep the interaction within the host's
supported question and option limits, and retain an `Other` or free-text route for base or manual
values when the surface supports it.

If the structured surface is unavailable, fall back to concise numbered or plain-text questions with
the same semantics. Do not fail the workflow, silently choose a workspace or base, or force a
verification/continuity question outside true no-argument guided mode merely because the native
question surface is unavailable.

Both new-task workspace modes require a resolved `base` unless repository instructions provide a
stricter local override. Task identity requires exactly one of:
...
Workspace must resolve to `checkout` or `worktree` from an explicit argument, clear prompt intent, or
user confirmation. If it is absent or contradictory, ask before switching or creating a branch or
worktree and report the resolution source. The default is `verification=agent`: run the maximum
feasible automated, runtime, browser, and interactive verification, then ask the user only for
genuinely user-only checks. `verification=balanced` adds mandatory user acceptance, while
`verification=user` leaves browser/manual verification to the user. Verification does not authorize
publishing.
...
If the resolved workspace is `checkout`, keep the current physical Git root and prepare its task
branch from the recorded base unless repository instructions provide a visible override. Do not
provision ignored files or claim a separate worktree runtime. A new checkout task requires no
unrelated tracked or untracked changes; a matching unfinished task may resume its continuity state.
When no active state exists, use the shared branch-aware parked-state discovery rules before treating
an existing branch as a new task. If the state belongs to another unfinished task, stop and ask
whether to finish, park, or abandon it.
Read the shared lifecycle and skip the worktree-only sections below.
...
project-specific checkout/worktree rule and report the override in the resolved echo. This developer
environment repository is identifiable by its root `.chezmoiroot` and keeps work in its primary
checkout.
...
Immediately before branch creation or provisioning, re-read both origin URLs and the base commit. If either remote
...
For `workspace=checkout`, remain in the current physical checkout and continue to the task-branch
step. Do not create or enter a worktree, provision ignored files, or claim a separate runtime. A
repository-specific instruction may forbid branch switching; report and follow that override.

For `workspace=worktree`, use the native Claude entry path below:
...
From the resolved workspace, verify its root, branch, HEAD, and status. They must equal the selected
workspace, task branch, recorded base commit, and a clean checkout apart from approved provisioned
files. Stop on any mismatch unless a reported repository-specific checkout override applies.
...
with `git -C` afterwards, and hold the applicable verification gate exactly where it was. A fresh session
...
the response, not only in a tool call. For a worktree, the session may have moved while the user's
editor has not; for checkout, identify the current physical checkout explicitly.
...
For a `worktree` workspace, removal is allowed only when all are true:
...
After publishing, run the required `task-continuity` completion gate before the final response,
