# Task continuity — what it is

Notes for the human. Agents read [SKILL.md](SKILL.md); nothing loads this file.

One markdown file, `.task-continuity/state.md`, private to one working directory. Claude Code,
...
against Git, and resumes. In native mode, invoke continuity explicitly. Say "continue from task
continuity" only to force that, or if the global bootstrap did not load.

The old `.project-continuity/` directory is accepted only as a one-time migration input. Move the
complete directory to `.task-continuity/` after confirming the canonical directory does not exist;
never keep both state directories active or create a `project-continuity` skill alias.
...
than overwritten — it moves to `.task-continuity/parked/<slug>.md` and moves back when you
return. `ls .task-continuity/parked/` lists all parked tasks. A worktree is still the
...
that named file. When a new task finds a completed active `state.md`, it runs the completion gate,
moves the old state to `.task-continuity/parked/` using a non-colliding name, and creates a fresh
active state when effective continuity is enabled. Completed state is never silently overwritten,
and the new task does not need `continuity=off` merely because the completed record is preserved.

New records also keep a short `Task` label, the task `Branch`, `Base branch`, immutable `Base commit`,
and compatibility `Started from` metadata. When no active state exists, a named current branch can
identify one parked record only when those fields form a unique match and the current request agrees
with its objective. Branch-only, legacy, ambiguous, detached-HEAD, or rebased-away matches require
explicit selection; the hook reports candidates but never moves state by itself.
