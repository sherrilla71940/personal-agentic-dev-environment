...
2. Apply the finished-state invariant: if `state.md` exists and `In progress`, `Next actions`,
   `Blockers`, and `TODO / deferred` are all empty or absent, the task is finished. For a terminal
   response that is closing this task, unless the Verification block already records
   `Cleanup: declined`, say that continuity looks unnecessary and offer cleanup immediately. Do
   not end the response with only a stale-state warning; after reconciling the warning, run this
   completion step in the same response. When this gate runs as part of the completed-state
   transition to a different task, report the finished invariant and continue with that transition;
   do not offer active-state deletion before parking the completed record.
...
- A new worktree starts with no continuity and must not inherit another task's state. Claude Code's
  ...
  decision is made. A pattern there matching `.task-continuity/` would leak one task's state
...
1. Check whether `.task-continuity/` is already ignored, and stop if it is.
2. Resolve the exclude file with `git rev-parse --git-path info/exclude`.
3. Add the anchored entry `/.task-continuity/`.
...
The canonical skill name is `task-continuity`, and the canonical state directory is
`.task-continuity/`. The former `.project-continuity/` directory is a migration input only; it is
not a supported second active state path. A legacy state is readable but must not be silently
copied, merged, overwritten, or deleted. If both directories exist, stop and resolve the collision
before continuing. To migrate an existing legacy state, confirm that `.task-continuity/` does not
exist, move the complete `.project-continuity/` directory—including `parked/`—to
`.task-continuity/`, and verify the new path is ignored. Do not create a skill alias for the removed
`project-continuity` name.

## Resume

Step 1 is a gate, not a formality. Everything after it assumes the answer was yes.

1. **After the repository identity preflight passes, read the `Task`, `Objective`, `Base commit`
   (or legacy `Started from`), and `Branch`, and decide whether this state tracks the task you were
   just asked to do.** If it does not, stop here and follow the wrong-task rules below.
...
- Replace an unfinished state only after the user explicitly abandons that task. A completed state
  follows the completed-state transition below, which preserves it by parking it before creating a
  fresh active state. A finished-state invariant by itself is not permission to overwrite, delete,
  or replace the record.
...
## Completed active state to a new task

When a different task arrives and the active `state.md` satisfies the finished-state invariant,
use this transition instead of treating the file as replaceable:

1. Reconcile the completed state and run the completion gate's invariant check. Because this is a
   transition to a different task, continue to parking instead of requesting active-state deletion
   confirmation first.
2. Preserve the completed state by moving it to `.task-continuity/parked/` with its `Parked:`
   timestamp. Use the objective-derived slug when available; if the destination already exists,
   add a timestamp or other non-colliding suffix. Never overwrite an existing parked file.
3. Initialize the new task with a fresh active `state.md` when its effective continuity policy is
   `on`. This preservation move does not require deletion confirmation and does not force the new
   task to use `continuity=off`.
4. Treat deletion of the parked completed record as a separate cleanup decision. Report it as a
   closure candidate and delete it only after the user confirms that named file.

This transition is separate from unfinished-task parking and applies in both `checkout` and
`worktree` workspaces. An unfinished state still requires an explicit finish, park, or abandon
decision before another continuity-enabled task starts. A completed state is preserved rather than
silently overwritten, but it must not block a later continuity-enabled task merely because cleanup
has not yet been requested.

## Branch-aware parked-state discovery

When the active state is absent and the current checkout is on a named branch, inspect
`.task-continuity/parked/` before treating that branch as a new task. A parked candidate is strong
only when all of these checks pass:

1. Its structured `Branch` exactly matches the current branch.
2. It records a non-placeholder `Task` label.
3. It records a non-placeholder `Base commit`, or the legacy `Started from` field.
4. That start commit is reachable from the current `HEAD`.

If exactly one strong candidate remains, compare its `Task`, `Objective`, base branch, and start
commit with the current request. If they describe the same task, select that exact file and restore
it. If more than one candidate remains, or only legacy/branch-only candidates exist, list the paths
and require an explicit file choice or user confirmation. A branch match alone never restores state.
Detached `HEAD`, a missing start commit, a rebased-away start commit, or a conflicting current
request also requires explicit reconciliation. The lifecycle hook may report these candidates, but it
does not move or rewrite files.

Before restoring a selected candidate, resolve any active `state.md` through the normal finish, park,
abandon, or completed-state transition. Then move the selected file back to `state.md`, remove its
`Parked` marker, preserve its `Task`, `Objective`, `Branch`, `HEAD`, `Base commit`, `Started from`,
and base metadata, and run the ordinary Resume workflow. Never copy over an active state or rewrite a
different parked file to make the match fit.
...
- **Resume:** use branch-aware parked-state discovery when the current branch provides a strong unique
  match. After explicit selection or a unique confirmed match, move the file back to `state.md`, remove
  `Parked`, and run the ordinary Resume workflow against it. Park whatever was active first; there is
  never more than one `state.md`.
...
The Git exclude entry is `/.task-continuity/`, so it already covers `parked/`. The lifecycle
...
`~/Documents/handoffs/{repo}/YYYY-MM-DD/{HH-mm}-{slug}.md`, record its `Created`, `Last updated`,
...
2. Reconcile against current repository and Git state. Capture the current Git root, `HEAD`, named
   branch (when present), working-tree status, and `git check-ignore -v -- .task-continuity/state.md`
   result in the same reconciliation. The recorded `HEAD` must come from that current checkout,
   not from an earlier session or an earlier command run. Report the state as "ignored by Git"
   unless the check explicitly proves the rule came from the global `core.excludesFile`; a local
   `.git/info/exclude` match is not a global-ignore claim.
   Obtain `Last reconciled` from the executing environment's current clock immediately before the
   whole-file write. Never estimate it, copy a conversation timestamp, or reuse an older value; if
   the clock cannot be read, stop before writing rather than inventing a timestamp.
...
`state.md` or a companion file under `~/Documents/handoffs/{repo}/YYYY-MM-DD/{HH-mm}-{slug}.md`. If the Artifact is the only copy,
...
`.task-continuity/`, or `.worktreeinclude`.
...
4. Delete `state.md` when active cleanup was confirmed, and delete only the specifically confirmed
   completed parked candidates. Remove the whole `.task-continuity/` directory only when
