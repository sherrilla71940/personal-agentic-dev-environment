...
The publish contract depends on the resolved workspace:

- `worktree` publishes the task branch against its recorded `base` branch after the exact-base
  freshness and integration gate below. The worktree remains available for the client-owned
  lifecycle after publication.
- `checkout` publishes the task branch from the current physical checkout after the same
  exact-base freshness and integration gate. It does not create a second worktree, but it still
  uses the recorded base and task branch established during invocation.
- A repository-specific override may narrow either workspace mode. It must be shown in the
...
Run this section for both `worktree` and `checkout` workspaces. The checkout workspace performs the
same checks from the current physical checkout; it does not gain worktree provisioning or a
separate runtime merely by reaching this section.
...
8. After a successful merge or rebase, rerun applicable automated verification. Because integration
   can change behavior after earlier verification, return to the selected verification policy and
   rerun the affected checks. Request fresh user acceptance only when that policy requires it or a
   genuinely user-only check remains.
...
- Attribute verification accurately: name the agent-run checks and separately state any user-only
  check or explicit user acceptance that was actually performed and reported as passing.
