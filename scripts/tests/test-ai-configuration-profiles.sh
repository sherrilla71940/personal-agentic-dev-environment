...
check_profile() {
  ...
  assert_contains "$invocation" 'pasted inline, no file'
  assert_contains "$invocation" 'offer once to file any received reference'
  assert_contains "$invocation" 'quote may open partway through a token'
  assert_contains "$invocation" 'Reject an unclosed quote'
  assert_contains "$invocation" 'The URL rule sits above the `=` rules'
  assert_contains "$invocation" 'same option repeated with different values'
  assert_contains "$invocation" 'shortcut consumes its one quoted prompt token'
  assert_contains "$invocation" 'material candidate instead of task text'
  assert_contains "$invocation" 'resolve to `plan`'
  assert_contains "$invocation" 'explicit requests to proceed, implement, migrate, or create resolve to'
  assert_contains "$project_continuity" 'could another supported client identify the objective'
  assert_contains "$project_continuity" 'Required external materials include deliverables'
  assert_contains "$invocation" 'record a reachable path or URL'
  assert_contains "$project_continuity" 'Re-read the file immediately before writing'
  assert_contains "$project_continuity" 'Never overwrite a version that was not just re-read'
  assert_contains "$project_continuity" 'Review every `parked/*.md`'
  assert_contains "$claude" 'terminal `/resume` picker'
  assert_contains "$claude" "VS Code extension's session"
  assert_contains "$claude" 'panel picker'
  assert_contains "$claude" 'new session'
  assert_contains "$claude" 'authoritative source and the rendered'
  assert_contains "$claude" 'target separately'
  assert_contains "$claude" 'subject repository'
